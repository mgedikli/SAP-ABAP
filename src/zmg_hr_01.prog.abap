...
*&---------------------------------------------------------------------*
REPORT ZMG_HR_01.
*HR_Personelmanagement
" 25.9. Loop At Group komutu
...
SELECT * FROM zmk_tablo_2 INTO TABLE @DATA(gt_tablo_2) WHERE id IN @so_id AND
                                                             yil = @p_yil.
DATA(lt_tablo_2) = gt_tablo_2[].
SORT lt_tablo_2 BY id.
...
LOOP AT lt_tablo_2 INTO DATA(ls_tablo_2).
  LOOP AT gt_tablo_2 INTO DATA(gs_tablo_2) WHERE id = ls_tablo_2-id.
      ...
      IF gv_name_sname IS INITIAL.
        READ TABLE gt_tablo_1 INTO DATA(gs_tablo_1)
                              WITH KEY id = gs_tablo_2-id.
