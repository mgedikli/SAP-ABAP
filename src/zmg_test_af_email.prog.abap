*&---------------------------------------------------------------------*
*& Report ZMG_TEST_AF_EMAIL
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zmg_test_af_email.

"ADOBE Form: PDF dokümanını e-mail olarak gönderme
...
START-OF-SELECTION.
  ...
  "Kendisine belirli bir bellek hakki taninmis e-mail gönderme sürecini baslatmak icin gerekli class objesini olustur.
  TRY.
  ...
  "Eklentinin PDF olacagini bildir; icerigini ve karakter sayisini ver. Ayrica, e-mail'in konusunu belirt.
  TRY.
...
*  "Aliciyi olustur.
  ...
  "E-maili gönder.
  TRY.
      DATA(gv_send_to_all) = go_send_request->send( i_with_error_screen = 'X' ).
    CATCH cx_send_req_bcs.
      BREAK-POINT.
  ENDTRY.
  ...
  IF gv_send_to_all = 'X'.
    MESSAGE 'E-Mail basarili bir sekilde gönderildi.' TYPE 'S' DISPLAY LIKE 'E'.
    COMMIT WORK.
  ELSE.
    MESSAGE 'E-Mail gönderme islemi basarisiz oldu.' TYPE 's' DISPLAY LIKE 'E'.
  ENDIF.
