...
*&---------------------------------------------------------------------*
REPORT zmg_test_331.
*Excel dosyalarını kullanma :=run time (dinamik)
*1-Excel dosyasını içe aktarma: kullanicidan excel dosya alip sistemde kodlama
*AT SELECTION-SCREEN ON VALUE-REQUEST FOR p_file.'F4_FILENAME' cagirma
...
"Kullanıcıdan dosya adını al / search help kulakcigi olustur
...
START-OF-SELECTION.
  ...
  CALL FUNCTION 'SPLIT_FILENAME'      "Kullanicinin girdigi dosya kontrol et; Excel mi? Bize "pure_extension" dosya uzantisi lazim.
    EXPORTING                         "'SPLIT_FILENAME' functiona yukaridan gelen 'p_file' dosya adini verebilmek icin 'long_filename'
                                      " parametresi ile ayni type bir parametreye ihtiyac var.
    ...
    IMPORTING                         " 'p_file' tipi yanlis->cöz: 'SPLIT_FILENAME' hangi tipte veri kabul eder?tikla/Import/DBMSGORA-FILENAME
